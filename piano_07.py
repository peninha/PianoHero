...
# Classe Note
class Note:
    def __init__(self, note, t, time_on, time_off, strength, piano, fall_speed=note_fall_speed, color=note_color):
        self.t0 = time_on
        self.t1 = time_off
        self.note = note
        ...
        self.x_coord = piano.get_note_x_coord(self.note)
...
# Classe Particle
class Particle:
    def __init__(self, x, y, color, life, size):
        self.x = x
        self.y = y
        ...
        self.life = life
        self.size = size
        self.velocity = [random.uniform(-0.5, 0.5), random.uniform(-1.5, -0.5)]  # Movimento ascendente com leve variação horizontal

    def update(self):
        self.x += self.velocity[0]
        self.y += self.velocity[1]
        self.life -= 1
        # Atualiza a cor para criar um efeito de fade-out
        fade_factor = self.life / 100  # Assuming initial life is 100
        self.color = (
...
class EffectManager:
    ...
    def add_particles(self, x, y, color, size, count=10):
        for _ in range(count):
            self.effects.append(Particle(x, y, color, 100, size))
...
class Piano:
    ...
    def generate_keys(self):
        ...
        for midi_note in range(start_midi, end_midi + 1):
            ...
            x_coord = x + whitekey_width/2 if note_type == "white" else x + blackkey_width/2
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    print("espaço")
                    effect_manager.add_particles(400, 300, (255, 255, 0), 1, 20)  # Adiciona 20 partículas amarelas
