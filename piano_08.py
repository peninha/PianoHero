...
class Efeitos:
    ...
    def add_faiscas_ascendentes(self, x, y, color, size, count=10):
        for _ in range(count):
            velocity = [random.uniform(-0.5, 0.5), random.uniform(-2.5, -0.5)]
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if key in piano.key_map:
                    piano.press_key(key)
                    #efeitos.add_glow(piano.get_note_x_coord(piano.key_map[key]['note']), keys_y, (0, 255, 255), 1)
                    for screen_note in screen_notes:
                        if (screen_note.note == piano.key_map[key]['note']) and (abs(screen_note.y_coord - endline_y_coord) <= press_distance_tolerance):
                            score += 1
                            efeitos.add_faiscas_ascendentes(piano.get_note_x_coord(piano.key_map[key]['note']), keys_y, (0, 255, 255), 1, count=20)
                if event.key == pygame.K_SPACE:
                    efeitos.add_glow(400, 300, (0, 255, 255), 1)
                    ...
                    #effect_manager.add_particles(400, 300, (255, 255, 0), 3, 20)
